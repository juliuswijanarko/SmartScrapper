...
def get_tr(request, user, pss):
	if request.method == 'GET':
		...
		with requests.Session() as c:
		    ...
		    useable = {
		    "availBalance": bal[10].get_text().strip()
		    }

		    return_data = {
		    "dataOf": month[1].get_text()[1:],
		    "accNo": month[4].get_text(),
		    "name": month[7].get_text(),
		    "period": month[10].get_text(),
		    "curr": month[13].get_text(),
		    "trns": list(),
		    "strtBal": month[len(month)-11].get_text(),
		    "totCrdts": month[len(month)-8].get_text(),
		    "totDbits": month[len(month)-5].get_text(),
		    "endBal": month[len(month)-2].get_text()
		    }

		    for itr in range (19, len(month)-14, 6):
		        return_data['trns'].append({
		        "date": month[itr].get_text()[1:],
		        "dtail": month[itr+1].get_text()[1:],
		        "brnch": month[itr+2].get_text()[1:],
		        "amnt": month[itr+3].get_text()[1:],
		    ...
		    "name": month1[7].get_text(),
		    "period": month1[10].get_text(),
		    "curr": month1[13].get_text(),
		    "trns": list(),
		    "strtBal": month1[len(month1)-11].get_text(),
		    "totCrdts": month1[len(month1)-8].get_text(),
		    "totDbits": month1[len(month1)-5].get_text(),
		    "endBal": month1[len(month1)-2].get_text()
		    }

		    for itr in range (19, len(month1)-14, 6):
		        return_data1['trns'].append({
		        "date": month1[itr].get_text()[1:],
		        "dtail": month1[itr+1].get_text()[1:],
		        "brnch": month1[itr+2].get_text()[1:],
		        "amnt": month1[itr+3].get_text()[1:],
		    ...
		    "name": month2[7].get_text(),
		    "period": month2[10].get_text(),
		    "curr": month2[13].get_text(),
		    "trns": list(),
		    "strtBal": month2[len(month2)-11].get_text(),
		    "totCrdts": month2[len(month2)-8].get_text(),
		    "totDbits": month2[len(month2)-5].get_text(),
		    "endBal": month2[len(month2)-2].get_text()
		    }

		    for itr in range (19, len(month2)-14, 6):
		        return_data2['trns'].append({
		        "date": month2[itr].get_text()[1:],
		        "dtail": month2[itr+1].get_text()[1:],
		        "brnch": month2[itr+2].get_text()[1:],
		        "amnt": month2[itr+3].get_text()[1:],
